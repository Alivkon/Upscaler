...
// Полки под работой: сколько работ на полке и сколько полок. Не размер
// указателя: указатель показывает всё. Полки ограничены, потому что стоят
// ниже сгиба и соревнуются за внимание с самой работой; что за полкой есть
// ещё, говорит ссылка на её страницу («See all»), а не длина полки.
// Шесть — потому что сетка ставит две колонки на телефоне и три на ноутбуке,
// и шесть заполняют ряды в обоих; при пяти каждая полка кончалась дыркой.
...
// тот, кто скачивает.
//
//   tall  — 9:16, карточка указателя, проём страницы работы и то, что
//           отдаёт Download; один файл на все три места
//   phone — 9:19.5, предлагается в `alternates` тому, у кого экран длинный
//   wide  — 16:9, рабочий стол; сейчас не отдаётся вовсе — см. HIDDEN_KINDS ниже
//
...
// Кадр 16:9 витрина не показывает — 12.09.2026, решение Charlie «lets hide
// desktop crops for now». Прячется весь кадр, а не мелкая его часть: до этого
// дня здесь стоял порог `DESKTOP_GATE` (1920 × 1080), и он убирал 40 кадров
// из 306, у которых мастер мелкий и до экрана не дотягивает.
//
// Файлы при этом остаются на диске и в манифесте, позиции — в
// `research/to-crop-desktop.md`: спрятан показ, а не работа. Возврат — убрать
// `wide` отсюда; всё, что кадр показывало, тем же и включится обратно.
//
...
//
// Плита при этом остаётся: у горизонтальной работы целая плита и есть
// рабочий стол, и подпись `alternates` говорит это про неё (187 работ витрины
// горизонтальны, 177 из них 4K и крупнее).
const HIDDEN_KINDS = ['wide'];

async function cropsOf(entry) {
  const found = {};
  for (const kind of CROP_KINDS) {
    if (HIDDEN_KINDS.includes(kind)) continue;
