...
Работе не нужно быть на витрине — хватит записи в `museum-works.json` (и `trim`, если у плиты рамка: лист режет её так же, как генератор). Лист кладётся в корень репозитория, а не в `/tmp`: там файлы стираются через десять дней.
