...
Горизонтальное фото — `desktop-wallpaper` вместо `iphone-wallpaper` в slug. **Только в slug.** Страница отдаёт телефонный кадр (`pages.js`, `offered`), и слово «desktop» в заголовке или alt пообещало бы файл, которого по кнопке не дают. Своё место у него есть — подпись к нетронутой плите в `alternates`, и там оно проставляется само. Подпись к 16:9 стоит рядом в коде, но кадр с 12.09.2026 спрятан (`HIDDEN_KINDS` в `gallery.js`).
...
**Источник** — файл-мастер ложится в `~/repos/wallpaper-gen/sources/<ref>.jpg`. TIFF только для Кливленда, где он есть; остальное — JPEG с сайта. Класть руками необязательно: адрес из поля `master` генератор скачивает сам при первом прогоне и оставляет копию там же.

**Обработка** — не выбирается. Работа без поля `treatment` выходит `dim` — умолчанием витрины с 12.09.2026, — и это верно для подавляющего большинства. Смотреть есть смысл тогда, когда результат не понравился:

```
cd ~/repos/Upscaler
node scripts/research/treat-sheet.mjs --only vl-0352,vl-0355
# → .treat-sheet.html в корне репозитория
```

Три плитки на работу: скан · dim · ceil. Значений у поля тоже три — поля нет (`dim`), `"ceil"` (то же плюс потолок яркости) и `"none"` (приглушённого файла у работы нет вовсе, страница показывает скан). Списки тех, кому пошли второе и третье, собраны двумя слепыми кругами: `research/2026-09-12-one-dim-rule.md`.
