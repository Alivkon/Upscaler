...
**Обработка** — выбирается глазами, на листе, до публикации. Без поля `treatment` работа выходит `dim`, но угадать заранее, кому что пойдёт, не получается: 15.09.2026 проход по всем 117 живым работам сменил правило у 59. Поэтому каждой партии новых картин — лист:

```
cd ~/repos/Upscaler
node scripts/research/treat-sheet.mjs --only vl-0352,vl-0355
xdg-open .treat-sheet.html
```

Работе не нужно быть на витрине — хватит записи в `museum-works.json` (и `trim`, если у плиты рамка: лист режет её так же, как генератор). Лист кладётся в корень репозитория, а не в `/tmp`: там файлы стираются через десять дней. Плитки лежат файлами в `.treat-sheet/` рядом с листом: окно в 1080 px по ширине весит слишком много, чтобы ехать строками base64 внутри HTML, — так что лист и его каталог носят вместе.

Три плитки на работу: скан · dim · ceil, под каждой кнопка — `none`, `dim`, `ceil`. Значений у поля тоже три — поля нет (`dim`), `"ceil"` (то же плюс потолок яркости 65) и `"none"` (приглушённого файла у работы нет вовсе, страница показывает скан). Выбор можно делать в несколько заходов, он переживает перезагрузку. Поле внизу собирает только те работы, где выбор расходится с нынешним значением, строками `vl-0352: ceil  (was dim)`; кнопка `Copy` копирует их. Эти строки и переносятся в `museum-works.json`: `ceil` и `none` — полем `"treatment"`, `dim` — удалить поле.

Правило стоит в имени файла (`-dim-`, `-ceil-`, `-none-`), поэтому сменить его можно и у вышедшей работы: сборка выпустит новые файлы, а старые под годовым кэшем не перезапишутся.

**На том же листе ставится и кадр.** Плитка — не вырезка, а проём: в ней лежит вся плита, окно 9:19.5 стоит на месте, картину внутри возят пальцем (мышью — тащить). Окно у трёх плиток общее, ездят они вместе. Сдвинутое окно приписывается к строке работы — `vl-0400: none  crop left 1578 top 0`, — и числа уже в тех пикселях, в которых `crop` записан в `museum-works.json`; кнопка ⊕ у названия возвращает окно туда, где оно стоит сейчас.

Но кадр на этом листе — прикидка, а не решение. Он здесь затем, чтобы было чем ответить на вопрос листа: картина, у которой в телефонном окне нет ничего, не нужна ни с каким правилом обработки. Окон при этом показано одно из трёх, и сравнить его не с чем. Кадр ставится следующим шагом, в `crop-positioner` (ниже), и поставленное там прикидку перекрывает.

**Рамка** — если у плиты физическая рамка, срезать перед кадрированием. Только рукой: автоматический резак снят 23.08, он оставлял рамку на каждой третьей работе. Обмер — `research/crop-ruler.html` (`node scripts/research/crop-ruler.mjs`), там показана целая плита и рулетка по всем четырём сторонам. Как и лист обработки, работе не нужно быть в каталоге: `--only=vl-0352,vl-0356` берёт её прямо из `museum-works.json`. Порядок целиком: `research/how-to-crop-frames.md`.
...
Лист кадрировки собирается заново: `node scripts/research/crop-positioner.mjs` — вся витрина, `--unseen` — только те, о ком в `to-crop-positions.md` строки ещё нет, `--desktop` — кадр 16:9 у тех, чей десктопный кадр витрина показывает (`research/to-crop-desktop.md`), `--only=vl-0399,vl-0407` — перечисление. Проём открывается там, где кадр стоит сейчас; вкладки наверху переключают три пропорции. Картинки лист берёт с витрины, так что `yarn start` должен быть поднят.

Как и лист обработки с линейкой рамки, `--only` не требует карточки в `catalogue/`: работа берётся прямо из `museum-works.json`, и кадр можно поставить до публикации. Нужна только запись в манифесте, то есть сборка (`museum.mjs`) — плиты лист не рисует, а показывает.

**Записали работу дважды — это два варианта, а не поправка.** Кнопка `record` добавляет строку, а не заменяет её, и переносить их надо все, по порядку: первая строка работы — `v1`, она уходит в `museum-works.json`, остальные ложатся в `to-crop-positions.md` строками без галочки. Свёртывается только буквальный повтор — то же число подряд. Лист не исправляют повторной записью, на нём перебирают: одна картина честно даёт два кадра, и выбирать между ними — отдельный разговор, для которого варианты и хранятся. Этот разговор стоит провести до публикации: `v1` — порядок записи, а не оценка, и листом вариантов (`scripts/research/variants-sheet.mjs`) видно, что именно встало в проём.
...
**На сервер** — `./scripts/deploy.sh`. Он сам запускает `yarn verify`, везёт код и картинки из манифеста и пересобирает контейнер; подробности — `DEPLOYMENT.md`.
