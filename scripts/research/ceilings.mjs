// Приглушение коллекции: два правила, `DIM` и `CEIL`; второе — это первое плюс
// потолок яркости.
//
// С 12.09.2026 умолчание витрины — `DIM`, и его же считает галочка в приёмке.
// `CEIL` остался у 11 работ, которым потолок яркости пошёл; списки — из двух
// слепых кругов, разбор в `research/2026-09-12-one-dim-rule.md`.
//
// Правило целиком:
//
...
//   3. приглушение ×0.70;
//   4. потолок средней цветности ≤18, а у `CEIL` ещё и средней яркости ≤57.
//
...
// картинки на одной галочке. Расхождение с генератором мерили 22.08 на всех 19
// работах, которые тогда несли `ceil`: худшее по множителю цвета было 0.044
// (vl-0030), по яркости 0.004, а на самой картинке — не больше 2 из 255
// у одного пикселя и 0.4 в среднем. С тех пор правил три, и заново это
// не мерили. Разбор: `research/2026-08-22-intake-treats-like-the-ticks.md`.
...
//
// У `DIM` потолка яркости нет вовсе, поэтому `b` у него всегда равен `dim`:
// приглушение ×0.70 достаётся и светлой работе, и тёмной. Это и есть разница
// между двумя правилами — `CEIL` темнит светлое сильнее.
export const DIM = { t: 0.55, dim: COLLECTION_DIM, capC: 18 };
export const CEIL = { ...DIM, capL: 57 };
 ...
 */
export function treat(data, width, height, rule = DIM) {
