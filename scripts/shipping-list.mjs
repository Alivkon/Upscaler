...
//
// Вторая версия работы — `scan` у той, что выходит приглушённой, `dimmed`
// у той, что выходит сканом (правило `none`), — устроена точно так же и потому
// собирается тем же обходом, а не своей веткой рядом. Разойдись такая ветка
// с этой, и галочка «Dimmed» на боевой машине переводила бы страницу на файл,
// которого там нет: вторую версию несёт каждая работа, и у неё свои копии
// и свои три кадра — почти половина списка.
//
// Поля два, а не одно, потому что они называют разное; обходятся оба, потому
// что выкладке всё равно, в какую сторону работает галочка, — ей важно, что
// файл на странице есть.
