...
// Кто и сколько раз может звать маршрут, у которого есть цена.
//
// Отдельный файл, а не строчка в `server.js`: у счёта своя причина
// существовать. Каждый вызов `/api/upscale` поднимает чужую видеокарту и стоит
// денег, а маршрут открыт всякому, кто откроет консоль браузера. До сих пор его
// защищал только `HOST=127.0.0.1`, и с публикацией эта защита кончается.
//
// Ворот у видеокарты трое, и они отвечают на три разных вопроса. Четвёртые,
// отдельные от них, стоят у рассылки: там цена не в деньгах, а в файле —
// см. `mailing` ниже и `mailingAllowance` в конце файла.
//
...
// выходит. Пятьдесят в сутки на всех — и есть та граница.
//
// Число выбиралось под платного Replicate, где вызов стоил пять центов и
// потолок держал расход в $75 в месяц. С 24.08 считает своя модель на чужой
// видеокарте, взятой на секунды, и вызов стоит $0.0044 — те же полсотни
// в сутки выходят в $6.58 в месяц. То есть потолок перестал быть границей
// расхода и остался тем, чем был на самом деле: границей злоупотребления.
// Двигать его теперь можно, и упрётся он не в деньги, а в то, сколько мы
// готовы считать за одного посетителя (TODO.md).
//
...
const RULES = {
  ...
  // Рассылка — ворота не про деньги, а про файл: маршрут `/mailing-list`
  // дописывает строку на диск, и открыт он всякому, кто умеет слать POST.
  // Пять в час с адреса — это заведомо больше, чем нужно человеку (ему хватит
  // одного), и заведомо меньше, чем нужно тому, кто решил налить нам файл.
  // Счёт только по адресу: кук эта страница не ставит и ставить не должна —
  // ради подписки на письма заводить опознавательный знак браузера незачем.
  mailing: {
    allowed: 5,
    window: HOUR,
    refusal: when => `Five addresses an hour from one connection is the limit. Try again in ${when}.`
  },
  server: {
    // Числа гостя и адреса названы в отказе словами, а это — нет, и нарочно:
    // общий потолок будет двигаться, а посетителю он всё равно ничего не
    // подсказывает — сколько именно осталось на всех, от него не зависит.
    // Так число живёт в одном месте.
    allowed: 50,
    window: DAY,
...
const marks = { visitor: new Map(), address: new Map(), server: new Map(), mailing: new Map() };
...
// пара расписок: `spend` — «деньги ушли», `release` — «до видеокарты не дошли».
//
...
//
// Считать при этом нужно вызовы видеокарты, а не попытки: приславший файл
// не того формата до неё не доходит и платить за него не должен —
...
// Разрешение оставить адрес. Ворота одни, и расписок у них нет: писать
// на диск — не то же, что звать видеокарту. Место занимается сразу и
// не возвращается: цена попытки здесь и есть цена записи, а отказ по
// неверному адресу должен стоить ровно столько же — иначе перебором
// заведомо неверных адресов ворота обходятся насквозь.
export function mailingAllowance(req) {
  const now = Date.now();
  const key = req.ip || 'unknown';
  const times = recent('mailing', key, now);
  if (times.length >= RULES.mailing.allowed) {
    const wait = freeAt('mailing', times) - now;
    return { refusal: RULES.mailing.refusal(inWords(wait)), retryAfter: Math.ceil(wait / 1000) };
  }
  hold('mailing', key, now);
  return {};
}
