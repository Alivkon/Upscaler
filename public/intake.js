...
// Начать заново. Не то же самое, что выбрать другой файл: диалог здесь не
// открывается вовсе, страница просто возвращается в тот вид, с которого
// начинают, — и файл в неё приносят любым из трёх способов.
//
// Разница не в удобстве, а в цене промаха. «Do another» стоит вплотную
// к Download, а готовая работа существует в одном экземпляре и уже стоила
// секунды чужой видеокарты: щелчок мимо кнопки открывал системное окно
// поверх страницы, и работа к тому времени была уже стёрта. Пустая приёмка
// стирает её так же, но говорит об этом сама собой и не уводит с сайта.
function startOver() {
  restored = null;
  opening.clear();
  renderEmpty();
