...
if (!comparable) {
  ...
  // Мастер страницей не загружается: `sizes` у проёма считается от высоты
  // рамки (pages.js, `plateSizes`) и выходит в несколько сотен пикселей,
  // а по srcset браузер на такую ширину берёт копию, а не мастер. Стеклу нужен именно мастер, иначе «1:1» будет
  ...
  // Плашку в проёме переключает галочка «Dimmed» (`dimmed.js`), и стекло
  // обязано ходить за ней. Иначе снявший галочку сравнивает скан музея
  // с приглушённым мастером — то есть ровно не то, ради чего стекло здесь
  // стоит: страница обещает разницу между «до» и тем, что показано, а
  // показывает разницу с тем, что показано было.
  //
  // Наблюдаем за `src`, а не слушаем галочку: стекло берёт то, что в проёме,
  // от кого бы подмена ни пришла, и о существовании галочки знать не обязано.
  new MutationObserver(() => {
    paint(afterPane, picture.src);
    // Заказан был прежний мастер; новый ещё нет, и следующее наведение
    // должно заказать его, а не счесть работу сделанной.
    warmed = false;
  }).observe(picture, { attributeFilter: ['src'] });
