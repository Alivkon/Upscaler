...
// Предложение реставрировать своё раскрывается один раз за посещение —
// сразу после того, как файл взяли. Один раз, а не на каждое скачивание:
// коллекцию смотрят подряд, и повторённая на пятой работе реплика из
// предложения превращается в преследование. Ровно этим «тактичное»
// и отличается от «всплывающего».
//
...
//
// Память — на посещение (`sessionStorage`), а не навсегда: сюда приходят
// раз в год, за это время предложение успевает стать новостью снова.
// Доступ обёрнут: в приватном окне и при запрещённых данных сайта
// обращение к хранилищу бросает исключение, а ронять из-за реплики
// остальной скрипт страницы нельзя.
const SEEN = 'tessarum:offer-seen';
...
if (offer && download) {
  download.addEventListener('click', () => {
    ...
    try {
      ...
      // хранилище закрыто — реплика просто покажется и в следующий раз
    }
    offer.classList.add('is-offered');
  });
}
