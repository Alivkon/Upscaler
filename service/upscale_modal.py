...
# моделью, которую выбрали вслепую: `4x_RealisticRescaler_100000_G`, лицензия
# WTFPL. С 25.08 она стоит здесь вместо `4x_NMKD-Siax_200k` — шесть пар из
# шести, картинка против картинки (research/2026-08-25-six-out-of-six.md).
# Siax выигрывал, пока версии показывались по одной: в одиночку резкая ветка
# читается как лучшая, а рядом с честной видно, что она дорисовывает.
# В браузере эта модель считать не может — как и прежняя, она на порядок
# медленнее браузерной и роняет вкладку; на боевой коробке видеокарты нет
# вовсе. То есть кроме как здесь считать её негде.
#
# Померено на Siax: холодный старт около 13 секунд сверх счёта, 0.47 с
# на плитку, $0.0044 за картинку вместе с полным холодным стартом. Модели
# одного семейства и одной скорости на процессоре (круг 15), но на T4 числа
# после замены не перемеряны.
...
MODEL = "4x_RealisticRescaler_100000_G.onnx"
...
# собралась бы вовсе. При старте — потому что 70 МБ на каждом холодном запуске
...
MODEL_SHA = "51773d75391906f1886bc99ce603e0ac7a7cc89fb4a77419c4bd63e466339d9c"
