...
// Блок под сеткой указателя: колонка на вид страниц (`rows` в browse.js),
// набранная как список треков на альбоме: подпись, под ней названия по одному
// в строку и счёт работ справа, где у трека длительность. Одной строкой
// двадцать ссылок читались облаком тегов, строками с подписями — панелью,
// которая ни на что не похожа; список альбома узнают сразу, и он тих.
//
...
const browseRows = rows =>
  rows.length
    ? `<nav class="browse" aria-label="Browse the collection">${rows
        .map(
          row =>
            `<div class="browse__group browse__group--${escape(row.label.toLowerCase())}"><p class="browse__label">${escape(row.label)}</p><ul class="browse__list">${row.links
              .map(
                link =>
                  `<li><a href="${escape(link.href)}"><span>${escape(link.text)}</span>${link.count ? `<span class="browse__count">${link.count}</span>` : ''}</a></li>`
              )
              .join('\n            ')}</ul></div>`
        )
        .join('\n          ')}</nav>`
...
const inPages = pages =>
  pages.length
    ? `<p class="in-pages">In ${pages
        .map(page => `<a href="${escape(page.path)}">${escape(page.name)}</a>`)
        .join(' · ')}</p>`
    : '';

// Тематическая страница — часть коллекции, показанная отдельным адресом:
// тема, традиция, страна или художник. Устройство и тексты тем и традиций —
// в `collections.js`, стран и художников — в `browse.js`; здесь только
...
// Ссылка — на страницу художника, а у кого работа одна — на неё саму
// (browse.js, `artistIndex`).
// Порядок — по имени, а не по фамилии, как в строке под сеткой: у японских
// имён фамилия первой, и правило пришлось бы задавать каждому из шестидесяти.
