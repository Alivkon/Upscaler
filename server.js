...
// Журнал запросов. Стоит выше всех маршрутов, потому что записать нужно и то,
// чего ни один маршрут не видел: файл из `public/`, ненайденный адрес, обход
// краулера. Правила записи и довод про баннер согласия — в `journal.js`.
//
// И выше `compression` — из-за столбца с весом. Обёртки `res.write` снимаются
// в порядке, обратном тому, в каком поставлены: кто зарегистрирован раньше,
// тот считает последним, уже после упаковки. Стоял бы журнал ниже — он мерил
// бы ответ до сжатия, то есть 184 436 байт там, где посетитель получил 23 045.
//
// Каталог задаётся снаружи: на боевой машине это том, переживающий пересборку
// (DEPLOYMENT.md), на своей — `log/` рядом с кодом.
app.use(journal(path.resolve(__dirname, process.env.LOG_DIR || 'log')));
...
// Чем берут файл: показывают или уносят. Раньше здесь стояла собственная
// запись о скачивании — она ушла в `journal.js`, где теперь записывается каждый
// запрос, а `Sec-Fetch-Dest` стоит отдельным столбцом. Довод остался тот же
// и стоит того, чтобы его помнить при чтении сводки.
//
// Картинку, стоящую в разметке, браузер просит с `image` — и страница работы,
// и лайтбокс: оба показывают файл через `<img>`. Всё остальное — уже не показ,
// а взятие файла самого по себе, и приходит оно разными пометками: сохранение
// и открытый в соседней вкладке адрес — разные жесты. Поэтому значение и
// пишется как есть: какое из них считать скачиванием, решает сводка.
//
// Недосчёт известен: у `plates` и `crops` стоит `max-age` в час
// (см. `setImageHeaders`), и повторное нажатие внутри этого часа до сервера
// не доходит вовсе. Ответ 304 при этом засчитан наравне с 200 — файл в этот
// момент уходит из кэша браузера на диск ровно так же.
