...
// Единственная запись о том, ради чего страница работы существует.
//
// Маяк Cloudflare считает загруженные страницы и только их: событий у него нет
// вовсе, а сайт не проксируется через Cloudflare — значит и статистики зоны не
// существует. Нажатие на «Download» поэтому до сих пор не считалось никак,
// и «сколько работ унесли» было вопросом без ответа
// (research/2026-09-12-HANDOVER-analytics.md).
//
// Считает сервер, а не браузер, по трём причинам. Файл и так отдаёт он —
// счёт не стоит ни строки на клиенте. Куки не ставятся, и вопрос баннера
// из LEGAL.md остаётся закрытым. И главное: маяк у части посетителей вырезан
// расширением — проверено на своём же браузере, две загрузки страницы не дали
// в панели ни одного просмотра, — а запрос за файлом вырезать нельзя, без него
// не будет и картинки.
//
// «Сохранил» от «посмотрел» отделяет `Sec-Fetch-Dest`. Картинку, стоящую
// в разметке, браузер просит с `image` — и страница работы, и лайтбокс: оба
// показывают файл через `<img>`. Всё остальное — это уже не показ, а взятие
// файла самого по себе, и таких значений несколько: сохранение и открытый
// в соседней вкладке адрес приходят разными. Поэтому значение пишется как
// есть, а не сводится к «да/нет»: это разные жесты, и какое из них считать
// скачиванием, решать по журналу, а не здесь. Замерено пока только то, что
// `image` — это показ; какой ровно пометкой приходит нажатие на «Download»,
// назовёт первая же строка журнала. На счёт это не влияет: считается всё,
// что не `image`. Совсем старый браузер заголовка не шлёт вовсе — такой
// запрос уходит с пометкой `none` и с прочими не смешивается.
//
// Пишется после ответа и только на успешный: несуществующее имя — не унесённая
// работа. Ответ 304 засчитан наравне с 200 — файл в этот момент уходит из кэша
// браузера на диск ровно так же. Обратное — недосчёт: у `plates` и `crops`
// стоит `max-age` в час (см. `setImageHeaders`), и повторное нажатие внутри
// этого часа до сервера не доходит вовсе.
app.use('/images', (req, res, next) => {
  const dest = req.get('sec-fetch-dest');
  if (dest !== 'image') {
    res.on('finish', () => {
      if (res.statusCode === 200 || res.statusCode === 304) {
        console.log(`download: ${dest || 'none'} ${decodeURI(req.path)}`);
      }
    });
  }
  next();
